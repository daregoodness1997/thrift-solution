...
circlesRouter.post("/", adminMiddleware, async (req, res) => {
  try {
    const {
      ...
      processingFeeType, processingFeeValue, clearanceFeeType, clearanceFeeValue, initialWeeksCount, defaultPenaltyType, defaultPenaltyValue,
    } = req.body;
    ...
    const circle = await createCircle({
      ...
      clearanceFeeType: clearanceFeeType === "fixed" || clearanceFeeType === "percent" ? clearanceFeeType : undefined,
      clearanceFeeValue: clearanceFeeValue != null && clearanceFeeValue !== "" ? Number(clearanceFeeValue) : undefined,
...
circlesRouter.put("/:id", adminMiddleware, async (req, res) => {
  try {
    ...
    const circle = await updateCircle(req.params.id, {
      ...
      clearanceFeeType: clearanceFeeType === "fixed" || clearanceFeeType === "percent" ? clearanceFeeType : (clearanceFeeType === null ? null : undefined),
      clearanceFeeValue: clearanceFeeValue != null && clearanceFeeValue !== "" ? Number(clearanceFeeValue) : (clearanceFeeValue === null ? null : undefined),
      initialWeeksCount: initialWeeksCount ? Number(initialWeeksCount) : undefined,
      defaultPenaltyType: defaultPenaltyType === "fixed" ? "fixed" : "percent",
      defaultPenaltyValue: defaultPenaltyValue != null && defaultPenaltyValue !== "" ? Number(defaultPenaltyValue) : undefined,
      status,

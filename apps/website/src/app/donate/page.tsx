  ...
  const handleDonate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/donations`, {
        method: "POST",
        headers,
        body: JSON.stringify({ amount: parseFloat(selectedAmount), provider, notes: notes || undefined }),
      });
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-brand-cream px-6 py-24">
        <div className="w-full max-w-md rounded-3xl border border-brand-primary/10 bg-white p-12 text-center shadow-sm">
          ...
          <h2 className="font-display text-2xl font-semibold text-brand-dark">Thank You!</h2>
          ...
          <button onClick={() => setSuccess(false)} className="mt-6 rounded-full bg-brand-primary px-8 py-3 text-sm font-semibold text-white">
            Donate Again
          </button>
        </div>
      </main>
      ...
      <section className="relative overflow-hidden px-6 pb-12 pt-6">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          <h1 className="font-display text-4xl font-bold tracking-tight text-brand-dark sm:text-5xl">
            Make a <span className="bg-gradient-to-r from-brand-primary to-brand-accent bg-clip-text text-transparent italic">donation</span>
          </h1>
      ...
      <section className="px-6 pb-12">
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {impactItems.map((item) => (
            <div key={item.amount} className="rounded-2xl border border-white/70 bg-white p-6 shadow-sm">
              <div className="mb-2 text-2xl">{item.icon}</div>
              <span className="block font-mono text-lg font-bold text-brand-primary">{item.amount}</span>
              <span className="mt-1 block text-xs font-light text-gray-500">{item.impact}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-gradient-to-b from-brand-surface/50 to-brand-cream px-6 py-16">
        <div className="mx-auto max-w-lg">
          <div className="mb-8 text-center">
            ...
            <h2 className="font-display text-2xl font-semibold text-brand-dark">Choose your contribution</h2>
          </div>

          <div className="rounded-3xl border border-brand-primary/10 bg-white p-8 shadow-sm">
            {error && (
              <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-xs font-medium text-red-600">{error}</div>
            )}

            <div className="mb-4">
              <label className="mb-1.5 block text-xs font-medium text-gray-700">Your Name</label>
              <input type="text" placeholder="Enter your name" value={name} onChange={(e) => setName(e.target.value)} className={inputClass} />
            </div>
            ...
            <div className="mb-4">
              <label className="mb-1.5 block text-xs font-medium text-gray-700">Select Amount</label>
            ...
            <div className="mb-4">
              <label className="mb-1.5 block text-xs font-medium text-gray-700">Payment Method</label>
              <div className="flex gap-2">
                {["paystack", "flutterwave", "nomba"].map((p) => (
                  <button
                    ...
                    className={clsx(
                      "flex-1 rounded-xl border px-3 py-2.5 text-xs font-semibold capitalize transition",
                      provider === p
                        ? "border-brand-primary bg-brand-primary/10 text-brand-primary"
                        : "border-gray-200 text-gray-500 hover:border-brand-primary/40"
                    )}
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-12 text-center">
            ...
            <h2 className="font-display text-3xl font-bold tracking-tight text-brand-dark">
              Hear from our <span className="bg-gradient-to-r from-brand-primary to-brand-accent bg-clip-text text-transparent italic">donors</span>
            </h2>
          </div>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {testimonials.map((t) => (
              <div key={t.name} className="rounded-2xl border border-white/70 bg-white p-7 shadow-sm">
                <p className="text-sm font-light italic leading-relaxed text-brand-dark">&ldquo;{t.text}&rdquo;</p>
                <div className="mt-5 flex items-center gap-3">
                  <div className="flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold text-white" style={{ background: t.color }}>
                    {t.name.charAt(0)}
                  </div>
                  <span className="text-xs font-semibold text-brand-dark">{t.name}</span>
                </div>
              </div>
            ))}
          </div>

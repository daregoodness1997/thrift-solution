    ...
    <div className="mx-auto max-w-[1280px] p-[clamp(1rem,3vw,2rem)]">
      ...
      <div className="mx-auto max-w-[1280px] p-[clamp(1rem,3vw,2rem)]">
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <ColorfulBadge
            label={TICKET_STATUS_CONFIG[ticket.status].label}
            color={TICKET_STATUS_CONFIG[ticket.status].color}
          />
          <ColorfulBadge
            label={TICKET_PRIORITY_CONFIG[ticket.priority].label}
        ...
        <Card className="mb-5 p-5">
          <p className="text-xs leading-relaxed text-brand-dark whitespace-pre-wrap">
            {ticket.description}
          </p>
        </Card>
        ...
        <div className="flex flex-col gap-3">
          {ticket.messages.map((m) => {
            ...
            return (
              ...
              >
                <div
                  className={`max-w-[80%] rounded-2xl px-4 py-3 ${mine ? "bg-brand-primary text-white" : "bg-white text-brand-dark"}`}
                >
                  <div
                    className={`mb-1 flex items-center gap-2 text-[10px] ${mine ? "text-white/70" : "text-gray-400"}`}
                  >

  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [listingsRes, receivedRes, sentRes] = await Promise.all([
        fetch(`${API_URL}/api/marketplace/my`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${API_URL}/api/marketplace/received-offers`, { headers: { Authorization: `Bearer ${token}` } }),
        fetch(`${API_URL}/api/marketplace/my-offers`, { headers: { Authorization: `Bearer ${token}` } }),
      ]);
      ...
      if (receivedData.success) setReceivedOffers(receivedData.data);
      if (sentData.success) setSentOffers(sentData.data);
    } catch {}
    setLoading(false);
  }, [token, API_URL]);

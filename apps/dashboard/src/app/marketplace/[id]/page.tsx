...
export default function ListingDetailPage() {
      ...
      <div className="mx-auto max-w-[960px] p-[clamp(1rem,3vw,2rem)]">
        <Card padding="3rem">
          <div className="text-center">
            <h3 className="mb-2 text-base font-semibold text-slate-900 dark:text-white">Listing not found</h3>
            <p className="mb-6 text-[13px] text-slate-500 dark:text-slate-400">This listing may have been removed.</p>
  ...
  return (
    <div className="mx-auto max-w-[960px] p-[clamp(1rem,3vw,2rem)]">
      <FadeIn>
        <div className="mb-6">
          <a href="/marketplace" className="text-xs font-semibold no-underline text-blue-600">&larr; Back to Marketplace</a>
        </div>
      </FadeIn>

      <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-8">
        <FadeIn delay={100}>
          <div className="relative flex aspect-square items-center justify-center overflow-hidden rounded-2xl bg-slate-50 dark:bg-slate-800/60">
            {listing.imageUrl ? (
        ...
        <FadeIn delay={200}>
          <div className="flex flex-col gap-4">
            <div className="flex flex-wrap gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-md bg-slate-100 dark:bg-slate-950 text-slate-700 dark:text-slate-300 text-[10px] font-mono font-bold uppercase tracking-wider">
                {listing.category}
              </span>
              {listing.status !== "active" && (
                <span className="px-2.5 py-0.5 rounded-md bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-300 text-[10px] font-mono font-bold uppercase tracking-wider">
                  {listing.status}
                </span>
              )}
            </div>

            <h1 className="text-[1.5rem] font-bold leading-[1.3] text-slate-900 dark:text-white">{listing.title}</h1>

            <div className="flex items-baseline gap-2">
              <span className="text-[1.75rem] font-mono font-extrabold text-blue-600">
                {formatNaira(listing.price)}
              </span>
              <span className="text-[11px] text-slate-500 dark:text-slate-400">{listing.currency}</span>
            </div>

            <div className="rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-800/60 p-4">
              <p className="whitespace-pre-wrap text-[13px] leading-[1.7] text-slate-900 dark:text-white">{listing.description}</p>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <div>
                <span className="block text-xs font-semibold text-slate-900 dark:text-white">{listing.seller.name}</span>
                <span className="text-[10px] text-slate-500 dark:text-slate-400">Listed {new Date(listing.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}</span>
              </div>
            </div>

            {isSeller ? (
              <div className="flex gap-3 pt-2">
                <a href={`/marketplace/${id}/edit`} className="btn-primary flex-1 text-center py-2.5 text-[13px]">
                  Edit Listing
                </a>
       ...
       {isSeller && listing.offers.length > 0 && (
        <FadeInUp delay={300}>
          <div className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-6">
            ...
            <h3 className="font-display font-bold text-xl sm:text-2xl text-slate-900 dark:text-white mt-1">Received Offers ({listing.offers.length})</h3>
            <div className="flex flex-col gap-3 mt-4">
              {listing.offers.map((offer) => (
                <div key={offer.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-800/60 p-4">
                  <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {offer.status === "pending" && listing.status === "active" && (
                      <div className="flex gap-1.5">
                        <button onClick={() => handleOfferAction(offer.id, "accepted")} className="btn-primary py-1.5 px-3 text-[11px]">Accept</button>
                        <button onClick={() => handleOfferAction(offer.id, "rejected")} className="btn-secondary py-1.5 px-3 text-[11px] text-red-600 border-red-200 dark:border-red-800">Reject</button>
                      </div>
                    )}
                  </div>
                </div>

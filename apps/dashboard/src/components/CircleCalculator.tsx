...
export function CircleCalculator({ circleAmount = 10000, annualRate = 10, onSelectConfig }: CircleCalculatorProps) {
  ...
  const totalInvestment = circleAmount * effectiveAccounts;

  const results = useMemo(() => {
    const weeklyPerAccount = calcWeeklyInterest(circleAmount, annualRate);
    const totalWeeks = Math.round((duration / 12) * 52);
    const totalInterestPerAccount = weeklyPerAccount * totalWeeks;
    const maturityPayoutPerAccount = circleAmount + totalInterestPerAccount;
  ...
  }, [circleAmount, annualRate, duration, effectiveAccounts]);

  const principalPct = (totalInvestment / results.totalPayout) * 100;
  const interestPct = (results.totalInterest / results.totalPayout) * 100;
        ...
        <div style={{ padding: "0 2rem 2rem" }}>
          <div style={{ marginBottom: "1.5rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "0.5rem" }}>
              ...
              <span style={{ fontSize: "18px", fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: cfg.colors.primary }}>{formatNaira(circleAmount)}</span>
            </div>
            <div style={{ padding: "0.75rem 1rem", borderRadius: "0.75rem", backgroundColor: "#1A1A1A", border: "1px solid #2A2A2A" }}>
              <span style={{ fontSize: "11px", color: "#666666" }}>Fixed deposit amount per circle account</span>
            </div>
          </div>
          ...
          <button onClick={() => onSelectConfig?.(circleAmount, duration, effectiveAccounts)}
        ...
        <div style={{ padding: "0 2rem 2rem 0", display: "flex", flexDirection: "column" }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
            <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
              <span style={{ display: "block", fontSize: "10px", fontWeight: 600, color: "#666666", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "0.375rem" }}>Maturity Payout</span>
              <span style={{ fontSize: "2rem", fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: "#ffffff" }}>{formatNaira(results.totalPayout)}</span>
            </div>
            ...
            <div style={{ display: "flex", gap: "1.5rem", justifyContent: "center", marginBottom: "1.5rem" }}>
              ...
              <div style={{ textAlign: "center" }}><span style={{ display: "block", fontSize: "10px", color: "#666666", marginBottom: "0.125rem" }}>You Earn</span><span style={{ fontSize: "14px", fontWeight: 600, fontFamily: "'JetBrains Mono', monospace", color: "#10B981" }}>{formatNaira(results.totalInterest)}</span></div>
            </div>
            ...
            <div style={{ padding: "1rem", borderRadius: "0.75rem", backgroundColor: "#1A1A1A", border: "1px solid #2A2A2A" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", marginBottom: "0.5rem" }}><span style={{ color: "#666666" }}>Total Interest (all accounts)</span><span style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 600, color: "#10B981" }}>{formatNaira(results.totalInterest)}</span></div>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", marginBottom: "0.5rem" }}><span style={{ color: "#666666" }}>Interest Rate</span><span style={{ fontWeight: 600, color: "#E0E0E0" }}>{annualRate}% p.a.</span></div>
